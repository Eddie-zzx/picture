...
import com.graduation.picture.enums.SpaceRoleEnum;
...
//                dynamicShardingManager.createSpacePictureTable(space);
    ...
    @Override
    public QueryWrapper<Space> getQueryWrapper(SpaceQueryQo spaceQueryQo) {
        QueryWrapper<Space> queryWrapper = new QueryWrapper<>();
        if (spaceQueryQo == null) {
            return queryWrapper;
        }
        // 从对象中取值
        Long id = spaceQueryQo.getId();
        Long userId = spaceQueryQo.getUserId();
        String spaceName = spaceQueryQo.getSpaceName();
        Integer spaceLevel = spaceQueryQo.getSpaceLevel();
        String sortField = spaceQueryQo.getSortField();
        String sortOrder = spaceQueryQo.getSortOrder();
        Integer spaceType = spaceQueryQo.getSpaceType();
        // 拼接查询条件
    ...
    @Override
    public boolean isSpaceCreator(Long spaceId, Long userId) {
        boolean exists = baseMapper.isSpaceCreator(spaceId, userId);
        return exists;
    }
}
